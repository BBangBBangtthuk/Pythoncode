...
def load_data(file_path=r"C:\Users\chohs\BMS\아이오닉5\01241227999\01241227999\2024-02-27"):
    global data 
    data = pd.read_csv(file_path)
    
    
def get_file_list(folder_path = r"C:\Users\chohs\BMS\아이오닉5\01241227999\01241227999\2024-02-27" , file_extension='.csv'):
    file_lists = [f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f)) and f.endswith(file_extension)]
    file_lists.sort()
    return file_lists

def process_files_trip_by_trip(file_lists, folder_path, save_path = r"C:\Users\chohs\BMS\아이오닉5\01241227999\01241227999\Save_file"):
    global data
...
def process_files_combined(file_lists, folder_path, save_path):
    for file in tqdm(file_lists):
        ...
        if 'altitude' in df.columns:
            ...
            data_save = df[['time', 'speed', 'acceleration', 'ext_temp', 'int_temp', 'soc', 'soh', 'chrg_cable_conn', 'altitude', 'pack_current', 'pack_volt', 'Power_IV','batt_coolant_inlet_temp','batt_internal_temp','batt_pw','cell_volt_list','mod_temp_list']].copy()
        else:
            # merge selected columns into a single DataFrame
            data_save = df[['time', 'speed', 'acceleration', 'ext_temp', 'int_temp', 'soc', 'soh', 'chrg_cable_conn', 'pack_current', 'pack_volt', 'Power_IV','batt_coolant_inlet_temp','batt_internal_temp','batt_pw','cell_volt_list','mod_temp_list']].copy()
        ...
        folder_path = r"C:\Users\chohs\BMS\아이오닉5\01241227999\01241227999\2024-02-27"
        file_lists = get_file_list(folder_path)
        
        process_files_trip_by_trip(file_lists, folder_path)
